...
// [[Rcpp::export]]
NumericVector mercator_to_linear_rcpp(NumericVector img, double fl_FF_mm, double zoom = 1.0) {
    ...
    for(int c_out = 0; c_out < W_out; c_out++) {
        ...
        // La coordenada angular theta en la proyección rectilínea/lineal (salida)
        double theta = std::atan(x / f_pixel_out);

        // Mapeo a la coordenada horizontal en la proyección Mercator original (entrada)
        double c_in = f_pixel_in * theta + cx_in;
        ...
        for(int r_out = 0; r_out < H_out; r_out++) {
            ...
            // Denominador del rayo 3D de la proyección rectilínea usando f_pixel_out
            double denom = std::sqrt(x2 + y_out_sq[r_out] + f_pixel_out * f_pixel_out);
            ...
            // Mapeo a la coordenada vertical en la proyección Mercator original (entrada)
            double r_in = f_pixel_in * v + cy_in;
