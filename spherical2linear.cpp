...
// [[Rcpp::export]]
NumericVector spherical_to_linear_rcpp(NumericVector img, double fl_FF_mm, double zoom = 1.0) {
    ...
    for (int c_out = 0; c_out < W_out; ++c_out) {
        ...
        for (int r_out = 0; r_out < H_out; ++r_out) {
            ...
            // Ángulo incidente desde el eje óptico inverso usando arctan de la proyección lineal de salida
            double theta = std::atan(R_out / f_pixel_out);
            
            // En la imagen de perspectiva esférica de entrada, R responde a f * theta
            double R_in = f_pixel_in * theta;
